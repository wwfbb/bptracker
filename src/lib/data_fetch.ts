...
export async function fetchData(timeFrame: TimeFrame) {
  ...
  for (const day of labels_deduped) {
    const day_records = data.filter(record => { return DateTime.fromSQL(record.date).toFormat("LLL dd") == day })

    const morning_record = day_records.filter(record => { return Number(record.time) == Time.MORNING })[0]
    ...
    const afternoon_record = day_records.filter(record => { return Number(record.time) == Time.AFTERNOON })[0]
    ...
    const night_record = day_records.filter(record => { return Number(record.time) == Time.NIGHT })[0]
