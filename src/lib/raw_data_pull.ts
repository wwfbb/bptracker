...
export async function getPage(page: number) {
    const { data, error } = await supabase.from("record").select().eq("user_uid", get(currentUserUUID)).order("date", { ascending: false }).order("time", { ascending: false }).range((page - 1) * PAGE_SIZE, (page * PAGE_SIZE) - 1);
